...
def random_merchant_category():
    array_merch_catg = [5732, 5944, 4829, 5678, 2314, 9487, 1234]
    merch_catg = random.choice(array_merch_catg)
    return merch_catg

# Generate purchase code
def random_purchase_code():
    array_purch_catg = ["3490", "0480", "1000", "4509", "2345", "9012"]
    prch_code = random.choice(array_purch_catg)
    return prch_code
 

# Write to a Data file
def write_file(number_acct_fields, number_trxn_fields, file_name):
    ...
    for i in range(0, number_acct_fields):
        acct_num = acct_num = random_account_number()
        
        num_trx = random.randint(0, number_trxn_fields)
        for j in range(0, num_trx):
...
def main ():
    ...
    # On average there will be acct * trxn/2 fieldsc
    number_acct_fields = 150
    number_trxn_fields = 15
    
    

    # Write all the data fields to a Transaction file.
    write_file(number_acct_fields, number_trxn_fields, file_name)
